...
-- Add subscription and domain fields to organizations if they don't exist
DO $$ BEGIN
  ALTER TABLE public.organizations ADD COLUMN subscription_tier public.subscription_tier NOT NULL DEFAULT 'free';
EXCEPTION
...
DO $$ BEGIN
  ALTER TABLE public.organizations ADD COLUMN max_users integer NOT NULL DEFAULT 5;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
  ALTER TABLE public.organizations ADD COLUMN max_units integer NOT NULL DEFAULT 3;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
  ALTER TABLE public.organizations ADD COLUMN subscription_started_at timestamp with time zone DEFAULT now();
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
  ALTER TABLE public.organizations ADD COLUMN subscription_expires_at timestamp with time zone;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

-- Create index for custom domain lookup
CREATE UNIQUE INDEX IF NOT EXISTS organizations_custom_domain_idx ON public.organizations(custom_domain) WHERE custom_domain IS NOT NULL;
